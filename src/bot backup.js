...
client.on('message', async (msg) => {
    ...
    if (state.step === 0) {
        ...
        if (!isValidFullName(fullName)) {
            await msg.reply("Nama lengkap tidak valid. Mohon sebutkan minimal 1 kata dengan benar.");
        } else {
        ...
        if (!isValidDate(birthDate)) {
            await msg.reply('Format tanggal lahir tidak sesuai, contoh "1992-08-25". Silahkan coba lagi.');
        } else {
            ...
            if (state.editing) {
                ...
                await msg.reply(
                    "Mohon Sebutkan Seluruh no registrasi nasional contohnya *3173051-5134*"
                );
                state.step = 3;
            }
        }
        ...
        if (!isValidNationalNumber(nationalNumber)) {
            await msg.reply(
                "Nomor Registrasi Nasional tidak valid. Format yang benar: 3173051-5134."
            );
        } else {
        ...
        if (!isValidWeight(weightInput)) {
            await msg.reply("Berat badan tidak valid. Mohon masukkan angka positif.");
        } else {
            ...
            if (state.editing) {
                ...
                await msg.reply(
                    "Mohon sebutkan Sisa Obat anda Hari ini ?"
                );
                state.step = 5;
            }
        }
        ...
        if (!isValidRemainingMedication(remainingMedicine)) {
            await msg.reply("Mohon sebutkan Sisa Obat anda Hari ini ?");
        } else {
